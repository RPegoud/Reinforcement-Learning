...
class Env:
    def __init__(self) -> None:
        ...
        self.nodes = {state: reward for state, reward in zip(self.states[1:], rewards)}
...
class Agent:
    def __init__(self) -> None:
        ...
        self.values = {
            state: value
            for state, value in zip(self.env.states, np.zeros(len(self.env.states)))
        }
    ...
    def update_value(self, s, s_prime, reward):
        ...
        if not self.done:
            self.values[s] = self.values[s] + self.alpha * (
                reward + self.gamma * self.values[s_prime] - self.values[s]
            )
        else:
...
def get_runs(alphas, n_runs=10, n_episodes=100):
    ...
    for alpha in alphas:
        ...
        for run in tqdm(range(n_runs)):
            ...
            dataframes[f"{alpha}"] = pd.concat(
                (dataframes[f"{alpha}"], pd.Series(errors)), axis=1
            )
    return dataframes
...
def plot_runs(dataframes):
    ...
    averaged_runs = pd.DataFrame(
        [average_run(dataframes[str(alpha)][0]) for alpha in alphas]
    ).T

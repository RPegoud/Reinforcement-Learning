...
from package.plots import plot_bar_chart, plot_heatmap
import numpy as np
...
class Q_learning_Agent(Agent):
    ...
    def fit(self, n_episode: int, plot_progress: list = None) -> None:
        ...
        @plot_progress (list): calls the plot_agent_performances function for
        each episode in the list
        """
    ...
    def plot_agent_performances(self) -> None:
        ...
        <span style='font-size: 13px'>Model parameters: [learning rate: {self.step_size},\
        epsilon: {self.epsilon}, discount: {self.gamma}]</span>"
